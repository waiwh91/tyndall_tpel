...
export const routes=['','research','people','news','publications','opportunities',...teamMembers.map(m=>'people/'+m.slug)];
export function pageTitle(page,lang='en'){return teamMembers.find(m=>'people/'+m.slug===page)?.name || translate(page?page[0].toUpperCase()+page.slice(1):'Home',lang);}
...
export function renderSite(page='',lang='en',base=''){
 ...
 markup=markup.replaceAll(`href="${base}/people/#cian-omathuna"`,`href="${pageUrl('people',lang,base)}#cian-omathuna"`);
 const languageLabel=({en:'Language',es:'Idioma','zh-CN':'语言',ar:'اللغة'})[lang];
 const currentLanguage=languages.find(([code])=>code===lang)[1];
 const selector=`<details class="language-dropdown" dir="ltr"><summary aria-label="${languageLabel}: ${currentLanguage}">${currentLanguage}<svg class="language-caret-icon" width="6" height="4" viewBox="0 0 6 4" aria-hidden="true" focusable="false"><path d="M0 0h6L3 4z" fill="currentColor"/></svg></summary><nav class="language-options" aria-label="${languageLabel}">${languages.map(([code,label])=>`<a href="${pageUrl(page,code,base)}" lang="${code}" hreflang="${code}"${code===lang?' aria-current="true"':''}>${label}</a>`).join('')}</nav></details>`;
