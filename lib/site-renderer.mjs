...
export function renderSite(page='',lang='en',base=''){
 ...
 const selector=`<details class="language-dropdown" dir="ltr"><summary aria-label="${languageLabel}: ${currentLanguage}">${currentLanguage}<span class="language-caret" aria-hidden="true"></span></summary><nav class="language-options" aria-label="${languageLabel}">${languages.map(([code,label])=>`<a href="${pageUrl(page,code,base)}" lang="${code}" hreflang="${code}"${code===lang?' aria-current="true"':''}>${label}</a>`).join('')}</nav></details>`;
