...
export function renderSite(page='',lang='en',base=''){
 ...
 const selector=`<nav class="language-switcher" aria-label="${({en:'Language',es:'Idioma','zh-CN':'语言',ar:'اللغة'})[lang]}" dir="ltr">${languages.map(([code,label])=>`<a href="${pageUrl(page,code,base)}" lang="${code}" hreflang="${code}"${code===lang?' aria-current="true"':''}>${label}</a>`).join('')}</nav>`;
